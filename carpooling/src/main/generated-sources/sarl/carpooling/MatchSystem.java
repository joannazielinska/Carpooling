...
import carpooling.Person;
import carpooling.SingleProposition;
import carpooling.Utils;
...
public class MatchSystem extends Agent {
  ...
  private void $behaviorUnit$Initialize$0(final Initialize occurrence) {
    ...
    for (final Integer i : _upTo) {
      ...
      _$CAPACITY_USE$IO_SARL_CORE_LIFECYCLE$CALLER.spawnInContext(Person.class, _$CAPACITY_USE$IO_SARL_CORE_INNERCONTEXTACCESS$CALLER.getInnerContext());
    }
    ArrayList<SingleProposition> _arrayList = new ArrayList<SingleProposition>();
    this.matches = _arrayList;
    ...
    MatchStart _matchStart = new MatchStart();
    _$CAPACITY_USE$IO_SARL_CORE_BEHAVIORS$CALLER.wake(_matchStart);
    ...
    final Procedure1<Agent> _function = (Agent it) -> {
      ...
      ArrayList<SingleProposition> matchesToErase = new ArrayList<SingleProposition>();
          ...
          for (final SingleProposition match : this.matches) {
            {
              ...
              if (((!Objects.equal(matchPeople, null)) && (matchPeople.size() >= 2))) {
                ...
                for (final SingleProposition mp : matchPeople) {
                  {
                    ArrayList<Address> others = new ArrayList<Address>();
                    for (final SingleProposition p : matchPeople) {
                      boolean _notEquals = (!Objects.equal(p, mp));
                      if (_notEquals) {
                        others.add(p.who);
                      }
                    }
                    ...
                    Matched _matched = new Matched(others, mp.startLocation, mp.destination);
                    final Scope<Address> _function_2 = (Address it_2) -> {
                      return Objects.equal(it_2, mp.who);
                    };
                    _$CAPACITY_USE$IO_SARL_CORE_BEHAVIORS$CALLER_1.wake(_matched, _function_2);
                  }
                }
                break;
              }
  ...
  @SyntheticMember
  @PerceptGuardEvaluator
  private void $guardEvaluator$Match(final Match occurrence, final Collection<Runnable> ___SARLlocal_runnableCollection) {
    assert occurrence != null;
    assert ___SARLlocal_runnableCollection != null;
    ___SARLlocal_runnableCollection.add(() -> $behaviorUnit$Match$1(occurrence));
  }
  ...
  @SyntheticMember
  public MatchSystem(final UUID parentID, final UUID agentID) {
  ...
  @Inject
  @Deprecated
  public MatchSystem(final BuiltinCapacitiesProvider provider, final UUID parentID, final UUID agentID) {
    super(provider, parentID, agentID);
  }
  
  @SyntheticMember
  @Inject
  public MatchSystem(final UUID parentID, final UUID agentID, final DynamicSkillProvider skillProvider) {
